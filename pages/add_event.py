...
with addCal:
    ...
    if st.button("Add Event"):
        ...
        if db.check_time_exists(start_datetime_str, end_datetime_str):
        ...
        # If the start time is less than the end time, add the event
            ...
            db.add_event(event_name, event_date, start_datetime_str, end_datetime_str)
...
with addHw:
    ...
    if st.button("Add Assignment"):
        ...
        if hw_title:
            db.add_hw(hw_title, hw_due_date)

    st.subheader("Current Assignments")
    df = pd.DataFrame(db.get_hw().iloc[:, 1:].to_numpy(), columns=list(["Assignments", "Due Date"]))
